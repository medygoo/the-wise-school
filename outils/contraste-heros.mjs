...
//      il en a autant que de points. L'aurore n'entre donc PAS dans le calcul, et
...
// Le héros vit maintenant dans `vivant.css` : le voile ET le fond du texte.
const CSS = readFileSync(join(RACINE, 'assets/vivant.css'), 'utf8');
...
const voileTxt = regle('.hero-premium>.hero-voile');
...
const panneauTxt = PREUVE ? 'background:rgba(17,20,22,0)' : regle('.hero-texte::before');

const aVoile = alphas(voileTxt, '.hero-premium>.hero-voile');
...
// garantit. `.hero-premium::after` (l'aurore) est donc décoratif et n'entre PAS dans le
...
const HALO = mFond ? +mFond[1] : (manques.push('.hero-texte::before — fond'), 0);

const TEINTE = [17, 20, 22];

// ── les textes du héros, avec leur opacité et leur taille réelles ──────────
// L'or de « exceller. » n'est pas UNE couleur : c'est un dégradé animé qui
// va de #f4d98a à #dcb043. On mesure le plus SOMBRE de ses arrêts — celui
// qui contraste le moins sur un fond sombre.
const OR = [0xdc, 0xb0, 0x43];
const TEXTES = [
  ['titre (h1)',          BLANC, 1,   42,   3.0],
  ["« exceller. » en or", OR,    1,   42,   3.0],
  ['sur-titre',           BLANC, .92, 15,   4.5],
  ['intro',               BLANC, .91, 17,   4.5],
  ['les trois repères',   BLANC, .84, 13,   4.5],
];
